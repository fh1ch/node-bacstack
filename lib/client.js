...
class Client extends EventEmitter {
  ...
  _performDefaultSegmentHandling(sender, adr, type, service, invokeId, maxSegments, maxApdu, sequencenumber, first, moreFollows, buffer, offset, length) {
    ...
    if (!moreFollows) {
      ...
      this._segmentStore = [];
      type &= ~baEnum.PduConReqBits.SEGMENTED_MESSAGE;
      this._handlePdu(adr, type, apduBuffer, 0, apduBuffer.length, null);
    }
  }
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length, npdu) {
    let result;
    ...
    if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
      ...
      let net = 0;
      let adr = [];
      if (npdu != undefined && npdu.source != undefined) {
        net = npdu.source.net;
        adr = npdu.source.adr;
      }
  ...
  _handlePdu(address, type, buffer, offset, length, npdu) {
    let result;
    ...
    switch (type & baEnum.PDU_TYPE_MASK) {
      case baEnum.PduTypes.UNCONFIRMED_REQUEST:
        ...
        this._processUnconfirmedServiceRequest(address, result.type, result.service, buffer, offset + result.len, length - result.len, npdu);
        break;
  ...
  _handleNpdu(buffer, offset, msgLength, remoteAddress) {
    ...
    this._handlePdu(remoteAddress, apduType, buffer, offset, msgLength, result);
  }
