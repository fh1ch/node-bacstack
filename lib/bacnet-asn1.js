...
var encodeContextBoolean = function(buffer, tagNumber, booleanValue) {
  ...
  buffer.buffer.writeUInt8(booleanValue ? 1 : 0);
  buffer.offset += 1;
};
...
var bacapp_decode_context_application_data = function(buffer, offset, max_offset, objectType, property_id) {
  ...
  if (IS_contextSpecific(buffer[offset])) {
    if (property_id === baEnum.BacnetPropertyIds.PROP_LIST_OF_GROUP_MEMBERS) {
      ...
      // TODO: Implement
      return;
      /*result = v.ASN1decode(buffer, offset, max_offset);
      if (result.tagLen < 0) {
        return;
      }
      ...
      result = decode_tag_number_and_value(buffer, offset + value.len);
      value.len++;
      if (result.tagNumber === 0) {
        result = decode_bacnet_time(buffer, offset + value.len);
        value.len += result.len;
        value.tag = baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_TIMESTAMP;
        value.value = result.value;
      } else if (result.tagNumber === 1) {
        result = decode_unsigned(buffer, offset + value.len, result.len);
        value.len += result.len;
        value.tag = baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_UNSIGNED_INT;
        value.value = result.value;
      } else if (result.tagNumber === 2) {
        result = decode_bacnet_datetime(buffer, offset + value.len);
        value.len += result.len;
        value.tag = baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_TIMESTAMP;
        value.len++;
        value.value = result.value;
    ...
    while ((value.len + offset) <= max_offset && !IS_CLOSING_TAG(buffer[offset + value.len])) {
      ...
      if (result.value === 0) {
        ...
        result = bacapp_decode_application_data(buffer, offset + value.len, max_offset, baEnum.BacnetObjectTypes.MAX_BACNET_OBJECT_TYPE, baEnum.BacnetPropertyIds.MAX_BACNET_PROPERTY_ID);
        if (result.tagLen < 0) {
          return;
        }
