...
module.exports.decode = function(buffer, offset) {
  ...
  var destination;
  if ((funct & baEnum.BacnetNpduControls.DESTINATION_SPECIFIED) === baEnum.BacnetNpduControls.DESTINATION_SPECIFIED) {
  ...
  var source;
  if ((funct & baEnum.BacnetNpduControls.SOURCE_SPECIFIED) === baEnum.BacnetNpduControls.SOURCE_SPECIFIED) {
  ...
  var networkMsgType = baEnum.BacnetNetworkMessageTypes.NETWORK_MESSAGE_WHO_IS_ROUTER_TO_NETWORK;
  var vendorId = 0;
  if ((funct & baEnum.BacnetNpduControls.NETWORK_LAYER_MESSAGE) === baEnum.BacnetNpduControls.NETWORK_LAYER_MESSAGE) {
    networkMsgType = buffer[offset++];
    if (networkMsgType >= 0x80) {
      vendorId = (buffer[offset++] << 8) | (buffer[offset++] << 0);
    } else if (networkMsgType === baEnum.BacnetNetworkMessageTypes.NETWORK_MESSAGE_WHO_IS_ROUTER_TO_NETWORK) {
      offset += 2;
    }
  }
...
module.exports.encode = function(buffer, funct, destination, source, hopCount, networkMsgType, vendorId) {
  ...
  buffer.buffer[buffer.offset++] = funct | (hasDestination ? baEnum.BacnetNpduControls.DESTINATION_SPECIFIED : 0) | (hasSource ? baEnum.BacnetNpduControls.SOURCE_SPECIFIED : 0);
  ...
  if ((funct & baEnum.BacnetNpduControls.NETWORK_LAYER_MESSAGE) > 0) {
