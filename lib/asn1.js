...
const decodeContextOctetString = (buffer, offset, maxLength, tagNumber, octetString, octetStringOffset) => {
  if (decodeIsContextTag(buffer, offset, tagNumber)) {
    const result = decodeTagNumberAndValue(buffer, offset);
    return {
...
const bitstringSetOctet = (bitString, index, octet) => {
  let status = false;
...
const decodeContextBacnetTime = (buffer, offset, tagNumber) => {
  const result = decodeIsContextTagWithLength(buffer, offset, tagNumber);
  if (!result.value) return;
  const decodedValue = decodeBacnetTime(buffer, offset + result.len);
  return {
...
const decodeContextDate = (buffer, offset, tagNumber) => {
  const result = decodeIsContextTagWithLength(buffer, offset, tagNumber);
  if (!result.value) return;
  const decodedValue = decodeDate(buffer, offset + result.len);
  return {
    len: result.len + decodedValue.len,
    value: decodedValue.value
  };
...
const bacappDecodeContextData = (buffer, offset, maxApduLen, propertyTag) => {
  let len = 0;
  let decodedValue;
  if (isContextSpecific(buffer[offset])) {
    let result = decodeTagNumberAndValue(buffer, offset);
