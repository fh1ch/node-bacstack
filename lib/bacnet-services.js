...
module.exports.DecodeAtomicReadFile = function(buffer, offset) {
  ...
  if (baAsn1.decode_is_opening_tag_number(buffer, offset + len, 0)) {
    isStream = true;
    len++;
    result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
    len += result.len;
    if (result.tagNumber !== baEnum.BacnetApplicationTags.BACNET_APPLICATION_TAG_SIGNED_INT) return;
    decodedValue = baAsn1.decode_signed(buffer, offset + len, result.value);
    len += decodedValue.len;
    ...
    if (!baAsn1.decode_is_closing_tag_number(buffer, offset + len, 0)) return;
    len++;
  } else if (baAsn1.decode_is_opening_tag_number(buffer, offset + len, 1)) {
    isStream = false;
    len++;
    result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
    len += result.len;
...
module.exports.EncodeAtomicReadFileAcknowledge = function(buffer, isStream, endOfFile, position, blockCount, blocks, counts) {
  ...
  if (isStream) {
    baAsn1.encode_opening_tag(buffer, 0);
    baAsn1.encode_application_signed(buffer, position);
    baAsn1.encode_application_octetString(buffer, blocks[0], 0, counts[0]);
    baAsn1.encode_closing_tag(buffer, 0);
  } else {
    baAsn1.encode_opening_tag(buffer, 1);
    baAsn1.encode_application_signed(buffer, position);
    ...
    for (var i = 0; i < blockCount; i++) {
      baAsn1.encode_application_octetString(buffer, blocks[i], 0, counts[i]);
    }
    baAsn1.encode_closing_tag(buffer, 1);
  }
};
...
module.exports.DecodeReadRange = function(buffer, offset, apduLen) {
  ...
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 0)) return;
  len++;
  decodedValue = baAsn1.decode_object_id(buffer, offset + len, 0);
  len += decodedValue.len;
  var objectId = {type: decodedValue.objectType, instance: decodedValue.instance};
  var property = {};
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  ...
  if (len < apduLen && baAsn1.decode_is_context_tag(buffer, offset + len, 2)) {
    result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
    len += result.len;
    decodedValue = baAsn1.decode_unsigned(buffer, offset + len, result.value);
    len += decodedValue.len;
  ...
  if (len < apduLen) {
    result = baAsn1.decode_tagNumber(buffer, offset + len);
    len += result.len;
    switch (result.tagNumber) {
      ...
      case 7:
        ...
        decodedValue = baAsn1.decode_application_date(buffer, offset + len);
        len += decodedValue.len;
        var tmpDate = decodedValue.value.value;
        decodedValue = baAsn1.decode_application_time(buffer, offset + len);
        len += decodedValue.len;
...
module.exports.DecodeReadPropertyAcknowledge = function(buffer, offset, apduLen) {
  ...
  var objectId = {};
  var property = {};
  if (!baAsn1.decode_is_context_tag(buffer, offset, 0)) return;
  var len = 1;
  result = baAsn1.decode_object_id(buffer, offset + len);
  len += result.len;
  ...
  var valueList = [];
  if (!baAsn1.decode_is_opening_tag_number(buffer, offset + len, 3)) return;
  len++;
  while ((apduLen - len) > 1) {
  ...
  if (!baAsn1.decode_is_closing_tag_number(buffer, offset + len, 3)) return;
  len++;
  return {
    len: len,
    objectId: objectId,
...
module.exports.DecodeReadPropertyMultiple = function(buffer, offset, apduLen) {
  ...
  var values = [];
  while ((apduLen - len) > 0) {
    var decodedValue = baAsn1.decode_read_access_specification(buffer, offset + len, apduLen - len);
...
module.exports.DecodeWriteProperty = function(buffer, offset, apduLen) {
  ...
  len++;
  var valueList = [];
  while ((apduLen - len) > 1 && !baAsn1.decode_is_closing_tag_number(buffer, offset + len, 3)) {
    decodedValue = baAsn1.bacapp_decode_application_data(buffer, offset + len, apduLen + offset, objectId.type, value.property.propertyIdentifier);
    if (!decodedValue) return;
    len += decodedValue.len;
...
module.exports.DecodeSubscribeCOV = function(buffer, offset, apduLen) {
  ...
  if (len < apduLen) {
    ...
    if (baAsn1.decode_is_context_tag(buffer, offset + len, 2)) {
    ...
    value.lifetime = 0;
    if (baAsn1.decode_is_context_tag(buffer, offset + len, 3)) {
      result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
      len += result.len;
      decodedValue = baAsn1.decode_unsigned(buffer, offset + len, result.value);
      len += decodedValue.len;
...
module.exports.DecodeSubscribeProperty = function(buffer, offset) {
  ...
  if (!baAsn1.decode_is_opening_tag_number(buffer, offset + len, 4)) return;
  len++;
  ...
  if (baAsn1.decode_is_context_tag(buffer, offset + len, 1)) {
    result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
    len += result.len;
    decodedValue = baAsn1.decode_unsigned(buffer, offset + len, result.value);
    len += decodedValue.len;
  ...
  if (!baAsn1.decode_is_closing_tag_number(buffer, offset + len, 4)) return;
  len++;
  value.covIncrement = 0;
  if (baAsn1.decode_is_context_tag(buffer, offset + len, 5)) {
    result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
    len += result.len;
    decodedValue = baAsn1.decode_real(buffer, offset + len);
    len += decodedValue.len;
...
module.exports.EncodeEventNotifyData = function(buffer, data) {
  ...
  baAsn1.encode_context_object_id(buffer, 1, data.initiatingObjectIdentifier.type, data.initiatingObjectIdentifier.instance);
  baAsn1.encode_context_object_id(buffer, 2, data.eventObjectIdentifier.type, data.eventObjectIdentifier.instance);
  baAsn1.bacapp_encode_context_timestamp(buffer, 3, data.timeStamp);
  ...
  if (data.messageText && data.messageText !== '') {
    baAsn1.encode_context_character_string(buffer, 7, data.messageText);
  }
  ...
  switch (data.notifyType) {
    case baEnum.BacnetNotifyTypes.NOTIFY_ALARM:
    case baEnum.BacnetNotifyTypes.NOTIFY_EVENT:
      baAsn1.encode_opening_tag(buffer, 12);
      switch (data.eventType) {
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_BITSTRING:
          baAsn1.encode_opening_tag(buffer, 0);
          baAsn1.encode_context_bitstring(buffer, 0, data.changeOfBitstringReferencedBitString);
          baAsn1.encode_context_bitstring(buffer, 1, data.changeOfBitstringStatusFlags);
          baAsn1.encode_closing_tag(buffer, 0);
          break;
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_STATE:
          baAsn1.encode_opening_tag(buffer, 1);
          baAsn1.encode_opening_tag(buffer, 0);
          baAsn1.bacapp_encode_property_state(buffer, data.changeOfStateNewState);
          baAsn1.encode_closing_tag(buffer, 0);
          baAsn1.encode_context_bitstring(buffer, 1, data.changeOfStateStatusFlags);
          baAsn1.encode_closing_tag(buffer, 1);
          break;
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_VALUE:
          ...
          switch (data.changeOfValueTag) {
            ...
            case baEnum.BacnetCOVTypes.CHANGE_OF_VALUE_BITS:
              baAsn1.encode_context_bitstring(buffer, 0, data.changeOfValueChangedBits);
              break;
            default:
              throw new Error('NotImplemented');
          }
          baAsn1.encode_closing_tag(buffer, 0);
          baAsn1.encode_context_bitstring(buffer, 1, data.changeOfValueStatusFlags);
          baAsn1.encode_closing_tag(buffer, 2);
          break;
        case baEnum.BacnetEventTypes.EVENT_FLOATING_LIMIT:
          baAsn1.encode_opening_tag(buffer, 4);
          baAsn1.encodeContextReal(buffer, 0, data.floatingLimitReferenceValue);
          baAsn1.encode_context_bitstring(buffer, 1, data.floatingLimitStatusFlags);
        ...
        case baEnum.BacnetEventTypes.EVENT_CHANGE_OF_LIFE_SAFETY:
          ...
          baAsn1.encode_context_bitstring(buffer, 2, data.changeOfLifeSafetyStatusFlags);
          baAsn1.encodeContextEnumerated(buffer, 3, data.changeOfLifeSafetyOperationExpected);
          baAsn1.encode_closing_tag(buffer, 8);
          break;
        case baEnum.BacnetEventTypes.EVENT_BUFFER_READY:
          baAsn1.encode_opening_tag(buffer, 10);
          baAsn1.bacapp_encode_context_device_obj_property_ref(buffer, 0, data.bufferReadyBufferProperty);
      ...
      baAsn1.encode_closing_tag(buffer, 12);
      break;
...
module.exports.DecodeEventNotifyData = function(buffer, offset) {
  ...
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 3)) return;
  len += 2;
  decodedValue = baAsn1.decode_application_date(buffer, offset + len);
  len += decodedValue.len;
  ...
  len += 2;
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 4)) return;
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decode_unsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  eventData.notificationClass = decodedValue.value;
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 5)) return;
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decode_unsigned(buffer, offset + len, result.value);
  len += decodedValue.len;
  ...
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 6)) return;
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decode_enumerated(buffer, offset + len, result.value);
  len += decodedValue.len;
  eventData.eventType = decodedValue.value;
  if (baAsn1.decode_is_context_tag(buffer, offset + len, 7)) {
    decodedValue = baAsn1.decode_context_character_string(buffer, offset + len, 20000, 7);
    len += decodedValue.len;
    eventData.messageText = decodedValue.value;
  }
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 8)) return;
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decode_enumerated(buffer, offset + len, result.value);
  len += decodedValue.len;
  ...
  switch (eventData.notifyType) {
    ...
    case baEnum.BacnetNotifyTypes.NOTIFY_EVENT:
      result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
      len += result.len;
      decodedValue = baAsn1.decode_unsigned(buffer, offset + len, 1);
      len += decodedValue.len;
      eventData.ackRequired = (decodedValue.value > 0);
      if (!baAsn1.decode_is_context_tag(buffer, offset + len, 10)) return;
      result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
      len += result.len;
      decodedValue = baAsn1.decode_enumerated(buffer, offset + len, result.value);
      len += decodedValue.len;
  ...
  if (!baAsn1.decode_is_context_tag(buffer, offset + len, 11)) return;
  result = baAsn1.decode_tag_number_and_value(buffer, offset + len);
  len += result.len;
  decodedValue = baAsn1.decode_enumerated(buffer, offset + len, result.value);
  len += decodedValue.len;
...
module.exports.EncodeReadRangeAcknowledge = function(buffer, objectId, propertyId, arrayIndex, resultFlags, itemCount, applicationData, requestType, firstSequence) {
  ...
  baAsn1.encode_context_bitstring(buffer, 3, resultFlags);
...
module.exports.DecodeReadRangeAcknowledge = function(buffer, offset, apduLen) {
  ...
  if (!(baAsn1.decode_is_opening_tag(buffer, offset + len))) return;
  len += 1;
...
module.exports.EncodeCOVNotify = function(buffer, subscriberProcessIdentifier, initiatingDeviceIdentifier, monitoredObjectIdentifier, timeRemaining, values) {
  baAsn1.encodeContextUnsigned(buffer, 0, subscriberProcessIdentifier);
  baAsn1.encode_context_object_id(buffer, 1, baEnum.BacnetObjectTypes.OBJECT_DEVICE, initiatingDeviceIdentifier);
  baAsn1.encode_context_object_id(buffer, 2, monitoredObjectIdentifier.type, monitoredObjectIdentifier.instance);
  ...
  values.forEach(function(value) {
    ...
    value.value.forEach(function(v) {
      baAsn1.bacapp_encode_application_data(buffer, v);
    });
    baAsn1.encode_closing_tag(buffer, 2);
