...
// Helper utils
var subscriptionService = function(invokeId) {
  var timer = setTimeout(function() {
    messageHandlers[invokeId] = false;
  }, 3000);
  messageHandlers[invokeId] = function() {
    cancelTimeout(timer);
  };
};
...
var receiveData = function(buffer, remoteAddress) {
  // Check data length
...
module.exports.readProperty = function(address, objectType, objectInstance, propertyId, arrayIndex) {
  ...
  var eventHandler = function(mInvokeId, timeout, next) {
    var timeout;
    var evtHandler = function(address, type, service, invokeId, buffer, offset, length) {
      if (mInvokeId === invokeId) {
        clearTimeout(timeout);
        var result = baServices.DecodeReadPropertyAcknowledge(buffer, offset, length);
        next(result);
        eventer.removeListener('complexAck', evtHandler);
      }
    };
    eventer.on('complexAck', evtHandler);
    timeout = setTimeout(function() {
      eventer.removeListener('complexAck', evtHandler);
      next({status: 'error', message: 'timeout'});
    }, timeout);
  };
  var maxSegments = 1;
  ...
  eventHandler(invokeId, 3000, function(result) {
    console.log(result);
  });
  transport.send(buffer.buffer, buffer.offset, address);
};

module.exports.readPropertyMultiple = function(address, propertiesArray, cb) {
  var maxSegments = 1;
  var buffer = getBuffer();
  baNpdu.Encode(buffer, baEnum.BacnetNpduControls.PriorityNormalMessage | baEnum.BacnetNpduControls.ExpectingReply, address, null, DEFAULT_HOP_COUNT, baEnum.BacnetNetworkMessageTypes.NETWORK_MESSAGE_WHO_IS_ROUTER_TO_NETWORK, 0);
  baAdpu.encodeConfirmedServiceRequest(buffer, baEnum.BacnetPduTypes.PDU_TYPE_CONFIRMED_SERVICE_REQUEST | (baEnum.maxSegments != baEnum.BacnetMaxSegments.MAX_SEG0 ? baEnum.BacnetPduTypes.SEGMENTED_RESPONSE_ACCEPTED : 0), baEnum.BacnetConfirmedServices.SERVICE_CONFIRMED_READ_PROP_MULTIPLE, maxSegments, transport.getMaxPayload(), invokeCounter++, 0, 0);
